...
const ClassesList = () => {
    ...
    const [selectedProfessor, setSelectedProfessor] = useState<string>("all");   // Bound to the teacher <Select>
    ...
    const classColumns = useMemo<ColumnDef<ClassListItem>[]>(
        () => [
            ...
            },
            {
                ...
                size: 200,
                header: () => <p className="column-title">Professor</p>,
                cell: ({ getValue }) => {
                    const professorName = getValue<string>();

                    return professorName ? (
                        <span className="text-foreground">{professorName}</span>
                    ) : (
    ...
    // Fetches only users with role=professor so the "Filter by professor" dropdown
    // only shows professor, not admins or students.
    const { query: professorsQuery } = useList<User>({
        resource: "users",
        filters: [
            {
                field: "role",
    ...
    // Sends ?professor=<name> to the backend when a professor is selected
    const professorsFilter =
    ...
    const classesTable = useTable<ClassListItem>({
        ...
        refineCoreProps: {
            ...
            filters: {
                ...
                permanent: [...subjectFilters, ...professorsFilter, ...searchFilters],
            },
            sorters: {
                initial: [
                    {
                ...
                <div className="actions-row">
                    ...
                    <div className="flex gap-2 w-full sm:w-auto">
                        ...
                        <Select value={selectedProfessor} onValueChange={setSelectedProfessor}>
                            ...
                            <SelectContent>
                                {/* "all" is the reset/default value — clears the professors filter */}
